...
function telegram_notification_for_whmcs_config() {
    $configarray = array(
    "name" => "ITFINDEN Telegram Notification for WHMCS",
    "description" => "Notifique de eventos por Telegram",
    "version" => "1.1",
    "author" => "ITFINDEN",
    "language" => "english",
    "fields" => array(
...
function telegram_notification_for_whmcs_deactivate() {
    $query = "DROP TABLE `ITNW`";
    $result = mysql_query($query);
}
function telegram_notification_for_whmcs_output($vars) {
    global $customadminpath, $CONFIG;
    $access_token = select_query('ITNW', '', array('adminid' => $_SESSION['adminid']));
    if ( $_GET['return'] == '1' && $_SESSION['request_token'] ) {
        
        insert_query("ITNW", array("adminid" => $_SESSION['adminid'], "access_token" => $result['access_token']));
        ...
        full_query("DELETE FROM `ITNW ` WHERE `adminid` = '".$_SESSION['adminid']."'");
        ...
        update_query('ITNW',array('permissions' => serialize($_POST['wt_notefication'])), array('adminid' => $_SESSION['adminid']));
