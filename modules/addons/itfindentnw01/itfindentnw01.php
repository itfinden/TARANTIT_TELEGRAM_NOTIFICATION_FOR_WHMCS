...
function itfindentnw01_output($vars) {
	...
	if ( $_GET['return'] == '1' && $_SESSION['request_token'] ) {
		...
		insert_query("db_itfindentnw01", array("adminid" => $_SESSION['adminid'], "access_token" => $result['access_token']));
		...
		header("Location: ". $CONFIG['SystemURL']."/".$customadminpath."/addonmodules.php?module=itfindentnw01&return=1");
	} elseif( $_GET['disable'] == '1' && mysql_num_rows($access_token) ) {
		full_query("DELETE FROM `db_itfindentnw01` WHERE `adminid` = '".$_SESSION['adminid']."'");
		...
		update_query('db_itfindentnw01',array('permissions' => serialize($_POST['wt_notefication'])), array('adminid' => $_SESSION['adminid']));
	...
	if ( !mysql_num_rows($access_token)) {
		echo "<p><a href='addonmodules.php?module=itfindentnw01&setup=1'>Activar el sistema para enviar notificaciones.</a></p>";
	} else {
		echo "<p><a href='addonmodules.php?module=itfindentnw01&disable=1'>Deshabilitar el sistema de notificación.</a></p>";
