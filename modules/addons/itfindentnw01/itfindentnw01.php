...
function itfindentnw01_output($vars) {
  ...
  if ( $_GET['return'] == '1' && $_SESSION['request_token'] ) {    
    ...
    if ($result['request_token'] && $result['redirect_url']) {
      ...
      echo "<div class='errorbox'><strong>Incorrect API Key</strong></br>Incorrect ITFINDEN API Key specified.  </div>";
    }
  ...
  if ( !mysql_num_rows($access_token)) {
    echo "<p><a href='addonmodules.php?module=itfindentnw01&setup=1'>Setup itfindentnw01</a></p>";
  } else {
    echo "<p><a href='addonmodules.php?module=itfindentnw01&disable=1'>Disable itfindentnw01</a></p>";
